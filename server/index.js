...
async function generateSingleMcqWithAi({
  ...
  for (let generationAttempt = 1; generationAttempt <= AI_SINGLE_MCQ_MAX_REGENERATIONS; generationAttempt += 1) {
    ...
    const runCompletion = async () => {
      const subjectLine = String(baseHierarchy.subject || '').trim() || 'Intelligence';
      const chapterLine = String(baseHierarchy.chapter || baseHierarchy.part || '').trim() || 'Critical Thinking';
      const sectionLine = String(baseHierarchy.section || baseHierarchy.topic || '').trim() || 'Problem Solving';
      const sourceTextReference = safeSourceText
        ? safeSourceText.slice(0, AI_PARSE_MAX_INPUT_CHARS)
        : (safeImageDataUrl ? '[image-uploaded-source]' : '[none]');
      const seenMcqsReference = existingReference || '[none]';

      const generationCommand = [
        'You are an expert MCQ examiner and test designer for competitive exams like NUST NET.',
        '',
        'Your task is to generate HIGH-QUALITY, EXAM-LEVEL MCQs that test deep understanding, not memorization.',
        '',
        '----------------------------------------',
        'CONTEXT:',
        `Subject: ${subjectLine}`,
        `Chapter: ${chapterLine}`,
        `Section/Topic: ${sectionLine}`,
        ...
        sourceTextReference,
        '',
        'Previously seen MCQs (do not repeat):',
        seenMcqsReference,
        '',
        '----------------------------------------',
        'OBJECTIVE:',
        '',
        `Generate ${safeRequestedCount} MCQ(s) based on the above context.`,
        '',
        'If no source content is provided, intelligently generate MCQs using your own knowledge of the subject, chapter, and topic.',
        '',
        '----------------------------------------',
        'STRICT RULES (VERY IMPORTANT):',
        '',
        '1. Each MCQ must include:',
        ...
        '   - Make options realistic and confusing (distractors must be strong)',
        '   - Avoid obvious wrong answers',
        '   - Avoid repetition of patterns',
        '   - Use exam-style wording (like real NET papers)',
        '',
        '3. DIFFICULTY CONTROL:',
        '   - EASY -> basic concept clarity',
        '   - MEDIUM -> application-based',
        '   - HARD -> multi-step reasoning or tricky logic',
        '',
        '4. UNIQUENESS:',
        '   - Do NOT repeat previously seen MCQs',
        '   - Do NOT rephrase common textbook questions',
        '   - Ensure each MCQ is UNIQUE',
        '',
        '5. STRUCTURE:',
        '   - Keep language clean and precise',
        '   - No unnecessary text',
        '   - No formatting errors',
        '',
        '----------------------------------------',
        'SPECIAL RULES (IMPORTANT FOR QUALITY):',
        '',
        '- If topic is Mathematics:',
        '  include calculations, expressions, or logic-based solving',
        '  avoid overly simple arithmetic',
        '',
        '- If topic is Intelligence / Logical Reasoning:',
        '  ensure logical consistency',
        '  conclusions must be separate and clear',
        '',
        '- If topic is English:',
        ...
        'NEVER DO:',
        '',
        '- Do NOT generate less than 4 options',
        '- Do NOT generate multiple correct answers',
        '- Do NOT leave any field empty',
        '- Do NOT generate vague or incomplete explanations',
        '- Do NOT copy from source text directly',
        '',
        '----------------------------------------',
        ...
        '',
        'Does each MCQ have exactly 4 options?',
        'Is only ONE option correct?',
        'Are options meaningful and non-repetitive?',
        'Is explanation logically correct?',
        'Is question aligned with difficulty level?',
        '',
        'If ANY check fails -> FIX the MCQ before returning.',
        '',
        '----------------------------------------',
      ...
      const userContent = [generationCommand, rejectionHint]
        .filter(Boolean)
        .join('\n\n');
...
async function generateMultipleMcqsWithAi({
  sourceText,
  imageDataUrl,
  instructions,
  difficulty,
  ...
  const targetCount = clamp(Number(count || AI_MULTI_MCQ_GENERATION_COUNT), 1, 15);
  const generated = [];
  const errors = [];
...
async function handleGenerateMcqs(req, res) {
  try {
    ...
    const difficulty = normalizeDifficulty(String(req.body?.difficulty || 'Medium').trim());

    const sourceType = sourceTypeRaw === 'file' || sourceTypeRaw === 'text'
      ? sourceTypeRaw
