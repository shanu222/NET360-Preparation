...
async function generateSingleMcqWithAi({
  ...
  for (let generationAttempt = 1; generationAttempt <= AI_SINGLE_MCQ_MAX_REGENERATIONS; generationAttempt += 1) {
    ...
    const runCompletion = async () => {
      ...
      const topicLine = String(baseHierarchy.topic || baseHierarchy.section || '').trim() || 'Problem Solving';
      const uploadedMcqsReference = safeSourceText
        ? safeSourceText.slice(0, AI_PARSE_MAX_INPUT_CHARS)
        : (safeImageDataUrl ? '[image-uploaded-mcqs]' : '[none]');
      const dbMcqsReference = existingReference || '[none]';

      const generationCommand = [
        'You are an expert NUST NET exam paper setter across multiple subjects.',
        '',
        '========================',
        'LEARNING DATA (MANDATORY)',
        '========================',
        '',
        '1. Previously generated high-quality MCQs (database):',
        dbMcqsReference,
        '',
        '2. MCQs from uploaded document:',
        uploadedMcqsReference,
        '',
        'Carefully analyze and LEARN:',
        '- Question patterns',
        '- Difficulty level',
        '- Structure',
        '- Common traps and logic',
        '',
        '========================',
        ...
        '- Follow real NUST NET patterns',
        '- Avoid theory-based or definition questions (unless subject requires it)',
        '- Questions must be concept-based and slightly tricky',
        '- Options must be close and competitive',
        '- Only ONE correct answer',
        '- Explanation must be short, logical, and educational',
        '',
        'SUBJECT-SPECIFIC RULES:',
        '',
        'Mathematics / Physics / Quantitative:',
        '- Numerical, calculation-based',
        '- Include formulas, logic, steps',
        '- Avoid direct formula recall questions',
        '',
        'Intelligence:',
        '- Logical reasoning, sequences, coding, deduction',
        '- Pattern-based, tricky, indirect',
        '',
        'English:',
        '- Vocabulary, grammar, comprehension logic',
        '- Use commonly confused words, sentence correction',
        '',
        'Chemistry / Biology:',
        '- Conceptual + application-based',
        '- Avoid pure memorization questions',
        '- Use real exam-style conceptual traps',
        '',
        'Computer Science:',
        '- Logic, algorithms, basic concepts',
        '- Avoid overly theoretical definitions',
        '',
        'Design Aptitude:',
        '- Visual reasoning, patterns, creativity-based logic',
        '',
        '========================',
        'TASK',
        '========================',
        '',
        `Generate ${safeRequestedCount} MCQs.`,
        '',
        '========================',
        ...
        '========================',
        '',
        '- DO NOT generate generic or easy textbook MCQs',
        '- DO NOT repeat patterns',
        '- MATCH the style of provided MCQs EXACTLY',
        '- Each question must require thinking (not direct recall)',
        `- Maintain consistent ${requestedDifficulty} level`,
        '- Reject weak questions internally before output',
      ].join('\n');

      const userContent = [generationCommand, userInstructions ? `Additional Instructions: ${userInstructions}` : '', rejectionHint]
        .filter(Boolean)
        .join('\n\n');
