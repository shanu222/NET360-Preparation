...
const DEFAULT_PROD_API_BY_HOST: Record<string, string> = {
  'www.net360preparation.com': `https://${CANONICAL_API_HOST}`,
  'net360preparation.com': `https://${CANONICAL_API_HOST}`,
};
...
export async function apiRequest<T>(path: string, options: ApiRequestOptions = {}, token?: string | null): Promise<T> {
  ...
  if (!response.ok) {
    // Single-device login: return 409 ACTIVE_SESSION_ELSEWHERE to the caller so the UI can show
    // a confirmation dialog. Do not auto-forceLogin here — that silently kicks the other device.
    if (
      response.status === 409
      && method === 'POST'
      && (path === '/api/auth/login' || path.split('?')[0] === '/api/auth/login')
    ) {
      // Fall through to standard error mapping with payload.code preserved.
    }
