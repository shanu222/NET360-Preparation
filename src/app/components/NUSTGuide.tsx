...
export function NUSTGuide() {
  ...
  const checkEligibility = () => {
    ...
    if (Number.isNaN(ssc) || Number.isNaN(hssc)) {
      toast.error('Please enter valid SSC and HSSC percentages.');
      setEligibilityResult([]);
      return;
    }

    if (ssc < 0 || ssc > 100 || hssc < 0 || hssc > 100) {
    ...
    if (!eligible.length) {
      toast.message('Currently not eligible for listed programs based on entered marks.');
    }
  };
