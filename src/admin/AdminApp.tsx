...
export default function AdminApp() {
  ...
  const runPasteMcqOcr = async (input: string) => {
  ...
  const formatOcrToMcqStructure = (rawText: string) => {
    ...
    ['A', 'B', 'C', 'D'].forEach((key) => {
      if (optionsByKey[key]) {
        lines.push(`${key}. ${optionsByKey[key]}`);
      }
    });
  ...
  const handlePasteMcqImagePaste = async (dataUrl: string) => {
    ...
    try {
      const extractedText = await runPasteMcqOcr(normalized);
      if (!extractedText.trim()) {
        toast.error('Could not extract readable MCQ text from pasted image.');
        return;
      }

      const formatted = formatOcrToMcqStructure(extractedText);
      setSingleMcqInput(formatted);
      toast.success('Pasted image converted and formatted as MCQ text.');
    } catch {
